...
public class DemoAuth implements Watcher {
    ...
    final static String SERVER_LIST = "192.168.126.134:2181";
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            zooKeeperClient = new ZooKeeper(SERVER_LIST, 50000,null);
            zooKeeperClient.addAuthInfo(authentication_type, correctAuthentication.getBytes());
        } catch (Exception e) {
        ...
        try {
            zooKeeperClient.create(PATH, "init content".getBytes(), acls,CreateMode.PERSISTENT);
            System.out.println("使用授权key：" + correctAuthentication + "创建节点：" + PATH + ", 初始内容是: init content");
        } catch (Exception e) {
            e.printStackTrace();
        }
        try {
            zooKeeperClient.create(PATH_DEL,"待删节点".getBytes(), acls, CreateMode.PERSISTENT);
    ...
    static void getDataByBadAuthentication() {
        ...
        try {
            ...
            zooKeeperClient = new ZooKeeper(SERVER_LIST, 50000,null);
            zooKeeperClient.addAuthInfo(authentication_type, badAuthentication.getBytes());
            System.out.println(prefix + "成功获取数据：" + new String(zooKeeperClient.getData(PATH,null,null),"UTF-8"));
        } catch (Exception e) {
    ...
    static void updateDataByNoAuthentication() {
        ...
        try {
            zooKeeperClient = new ZooKeeper(SERVER_LIST, 50000,null);
            if (zooKeeperClient.exists(PATH,null) != null) {
                zooKeeperClient.setData(PATH, prefix.getBytes(),1);
    ...
    static void updateDataByBadAuthentication() {
        ...
        try {
            ...
            if (zooKeeperClient.exists(PATH,false) != null) {
                zooKeeperClient.setData(PATH, prefix.getBytes(),0);
                System.out.println(prefix + "更新成功");
            }
        } catch (Exception e) {
            System.err.println(prefix + "更新失败，原因是：" + e.getMessage());
